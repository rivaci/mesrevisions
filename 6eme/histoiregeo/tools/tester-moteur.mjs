...
await test('un défi enregistre son meilleur score', () => {
  // L'identifiant vient du parcours plutôt que d'être écrit en dur : ces tests
  // valent pour le MOTEUR, quel que soit le programme qu'on y verse.
  const premiere = ETAPES[0].id;
  store.enregistrerDefi(premiere, 0.9);
  store.enregistrerDefi(premiere, 0.6);
  assert.equal(store.lireEtat().defis[premiere], 0.9, "un moins bon score n'écrase pas le meilleur");
});
