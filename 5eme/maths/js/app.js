...
function vueInstallation() {
  return `
    <header class="entete">
      <p class="surtitre">Mathématiques · 5<sup>e</sup></p>
...
function vueChapitres() {
  ...
  return `
    ...
    <p class="note">Les chapitres arrivent ici au fur et à mesure que tu les vois en
      classe : le numéro dit l'ordre où ils ont été ajoutés, pas une progression.
      Va directement à celui que tu viens de travailler, ou à celui que tu sais fragile.</p>`;
}
...
function vueExercice(sf) {
  ...
  if (ex.type === 'calcul') {
    ...
    // Un QCM dont les options appartiennent à l'exercice : la bonne écriture
    // d'une phrase, le nom d'un calcul, la nature d'une paire d'angles. Les
    // réponses fausses proposées sont les erreurs PRÉVUES, chacune reliée à
    // son piège : le choix de l'élève dit quelle confusion a joué.
    saisie = `${enonceExercice(ex)}
