    ...
    requete({ cle, modele, consignes, profil, message, schema, nomSchema }) {
      return {
        ...
        corps: {
          ...
          prompt_cache_key: `francais3e-${slug()}`,
    ...
    requeteChat({ cle, modele, consignes, profil, contexte, historique }) {
      ...
      return {
        ...
        corps: {
          ...
          stream: true,
          store: false,
          prompt_cache_key: `francais3e-chat-${slug()}`,
          reasoning: { effort: EFFORT },
...
const consignes = (prenom) => `Tu t'appelles Merlin. Tu es le professeur particulier de ${prenom}, 13 ans, en 3e, qui reprend chaque leçon au fur et à mesure qu'il la voit en classe.

Ton rôle n'est pas de réciter la règle ou la définition : c'est de lui faire
voir pourquoi il s'est fait avoir sur CETTE phrase-là, et de lui laisser un
test qu'il pourra refaire seul pendant le contrôle.
...
const consignesChat = (prenom) => `Tu es Merlin, le professeur particulier de ${prenom}, 13 ans, en 3e. Vous discutez, et ${prenom} peut te poser des questions.
