...
import { SEANCES, BLOCS, TOUS_EXERCICES, piegesUtilises } from '../js/data/seances/index.js';
...
// L'application se remplit au fil des cours : pas de nombre fixe de séances,
// mais chaque séance doit appartenir à un bloc déclaré.
if (!SEANCES.length) dire(erreurs, 'aucune séance');
for (const s of SEANCES) {
  if (!BLOCS.some((b) => b.numero === s.bloc && b.seances.includes(s.numero))) {
    dire(erreurs, `Séance ${s.numero} : absente de son bloc ${s.bloc} dans BLOCS`);
  }
}
