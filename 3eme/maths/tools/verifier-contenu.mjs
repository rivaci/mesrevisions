...
// --- Invariant 7 : l'antislash perdu ----------------------------------------
//
// Dans un fichier source JS, une commande LaTeX s'écrit avec DEUX antislashs :
// '\\times'. Avec un seul, JavaScript interprète l'échappement avant que le
// LaTeX n'existe, et le dégât dépend de la lettre qui suit :
//
//   '\ldots' → « ldots »          (échappement inconnu, l'antislash tombe)
//   '\times' → TAB + « imes »     (\t est une vraie tabulation)
//   '\frac'  → saut de page + « rac »
//
// Chercher « times » dans le fichier ne trouve donc RIEN dans le deuxième cas :
// il faut aussi guetter les caractères de contrôle. Ce double symptôme n'est
// pas une précaution théorique — c'est ainsi que « 2,7 ldots 2,54 » et un
// « dfrac » cassé sont arrivés à l'écran dans l'application de 6e.
//
// « dfrac » et « frac » sont listés tous les deux : chercher seulement « frac »
// attraperait « dfrac » cassé d'un côté et raterait de l'autre, puisque le « d »
// n'est pas un échappement. Le motif exige que la commande ne soit précédée ni
// d'un antislash ni d'une lettre — sinon « \dfrac » correct déclencherait
// l'alerte sur « frac » — et qu'elle ne soit suivie d'aucune lettre, sinon le
// mot français « fraction » déclenche l'alerte sur « frac », et « divisé » sur
// « div ».

const COMMANDES_LATEX = [
  'ldots', 'div', 'square', 'sqrt', 'cdot', 'leq', 'geq', 'approx',
  'dfrac', 'frac', 'times', 'text', 'quad', 'circ', 'rightarrow',
];
...
for (const ch of CHAPITRES) {
  for (const sf of ch.savoirFaire ?? []) {
    ...
    for (const item of aControler) {
      ...
      // Symptôme 2 : le caractère de contrôle laissé par \t, \f, \v, \b, \r.
      if (/[\t\f\v\b\r]/.test(enonce)) {
        dire(
          erreurs,
          `${ou} : caractère de contrôle dans l'énoncé — c'est un antislash `
